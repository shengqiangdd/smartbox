...
export default function BottomNav() {
  ...
  return (
    <nav className="relative z-20 border-t border-slate-700/50 bg-slate-900 lg:hidden" style={{ minHeight: '48px' }}>
      {/* ── 更多面板（向上展开） ── */}
      {moreOpen && (
        <div
          ref={panelRef}
          className="absolute bottom-full left-0 right-0 border-t border-slate-700/50 bg-slate-900 shadow-[0_-4px_20px_rgba(0,0,0,0.4)]"
      ...
      {/* ── 底栏 ── */}
      <div className={`grid ${aiEnabled ? 'grid-cols-7' : 'grid-cols-6'}`} style={{ minHeight: '48px' }}>
          ...
          return (
            ...
            >
              <div className="relative">
                <Icon
                  size={20}
                  className={isActive ? 'text-wrench-400' : 'text-slate-500'}
                />
                {item.id === 'ssh' && sshSessions.length > 0 && (
                  <span className="absolute -right-1.5 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-wrench-500 text-[7px] font-bold text-white">
                    {sshSessions.length}
                  </span>
                )}
              </div>
        ...
        >
          <div className="relative">
            ...
            {isMoreActive && (
              <span className="absolute -right-1 -top-1 h-2 w-2 rounded-full bg-wrench-400" />
            )}
          </div>
          <span
